...
with_defaults :scope => "source",
              :key_binding => "M1+M2+f10",
              :output => :discard do |bundle|
  require 'ruble/terminal'
  
  command t(:php_artisan_migrate) do |cmd|
    cmd.invoke {|context|  Ruble::Terminal.open("php artisan migrate", context.project.to_dir.path) }
  end
